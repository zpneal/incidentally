...
3. [Generative models](#generative)
    a. [Teams model](#team)
    b. [Groups model](#group)
    c. [Blau Space model](#blau)
4. [Utilities](#util)
    a. [Block models](#block)
5. [Empirical data](#data)
    a. [US Congress](#congress)
...
* [`incidence.from.probability()`](#probability), [`incidence.from.vector()`](#vector), and [`incidence.from.distribution()`](#distribution) generate an incidence matrices and bipartite graphs with given constraints on its fill/density and marginals/degrees.
...
* [`add.blocks()`](#block) adds a block structure or planted partition to an existing incidence matrix or bipartite graph.

* [`incidence.from.congress()`](#congress) constructs an incidence matrix representing US Congress legislators' sponsorship of bills.
...
Of course, different types of Beta distributions can be combined. For example, we can generate a $100 \times 100$ incidence matrix in which the row marginals are *approximately* **right-tailed**, but the column marginals are *approximately* **left-tailed**:
```{r, fig.show="hold", out.width="33%"}
I <- incidence.from.distribution(R = 100, C = 100, P = 0.2,
...
Given a social network among 15 people, we can simulate their recruitment by three (`k = 3`) new organizations, where there is a `p = 0.9` probability that an individual inside an organization's niche becomes a member:
```{r, fig.show="hold", out.width="40%"}
G <- erdos.renyi.game(15, .33)  #A random social network of 15 people, as igraph
I <- incidence.from.adjacency(G, k = 3, p = .90, model = "blau")  #Groups model
V(I)$shape <- ifelse(V(I)$type, "square", "circle")  #Add shapes
plot(G, layout = layout_with_mds(G), main="Social Network")
plot(I, layout = layout_as_bipartite(I), main="New Organizations")
```

The social network is plotted using a Multidimensional Scaling layout, and therefore shows the nodes' positions in the abstract Blau Space from which organizations recruit members. In this example, organization 18 recruits 3, 6, and 12 as members. This organization's niche is located near the upper-left of the space. People 6 and 12 were likely inside its niche and therefore readily recruited (here, with a 90% probability). Although person 3 is outside it's niche (i.e., quite different from the organization's typical member), they were also recruited.

[back to Table of Contents](#toc)

# Utilities {#util}
## Block models {#block}
The `add.blocks()` function shuffles an incidence matrix to have a block structure or planted partition while preserving the row and column marginals. For example, after generating a 100 $\times$ 100 incidence matrix with a density of .1, we can plant a partition in which the within-group `density = 0.7`. In this example, the first 50 row nodes and first 50 column nodes are assigned to block 1, while the second 50 row nodes and second 50 column nodes are assigned to block 2:
```{r, echo = TRUE, results = 'hide'}
I <- incidence.from.probability(R = 100, C = 100, P = .1)
blocked <- add.blocks(I, density = .7, 
                       rowblock = c(rep(1,50),rep(2,50)),
                       colblock = c(rep(1,50),rep(2,50)))
```
```{r}
...
In this example, the row objects are partitioned into two randomly-sized groups labeled `A` and `B`, and similarly the column objects are partitioned into two randomly-sized groups labeled `A` and `B`. There is a 0.7 probability of a 1 occuring between a row and column object belonging to the same group. Note that the row and column marginals are preserved.

[back to Table of Contents](#toc)

# Empirical data {#data}
Incidence matrices and bipartite graphs are very common. These functions generate such objects from empirical data available online.

## US Congress {#congress}
In the US Congress, the legislative process begins when a legislator in the House of Representatives or Senate introduces a bill for consideration. The legislator who introduces a bill is the *sponsor*, while other legislators can support the bill by joining as *co-sponsors*. These activities can be recorded in an incidence matrix **I** where $I_{ik} = 1$ if legislator *i* sponsored or co-sponsored bill *k*, and otherwise is 0. These data can be useful for constructing co-sponsorship networks, where legislators are connected if they sponsored the same bills, which can provide insight into political alliances and oppositions [@neal2014].

The `incidence.from.congress()` function makes it possible to construct such an incidence matrix for a given `session` and for given types of `bills`. For example:
```{r, echo = TRUE, results = 'hide'}
I <- incidence.from.congress(session = 115, types = "sjres", format = "data")
```
generates data on Joint Resolution bills introducted in the 115th session of the Senate. By specifying `format = "data"`, the resulting object is a list containing (1) the incidence matrix, (2) a data frame of legislator characteristics, and (3) a data frame of bill characteristics:
```{r}
I$matrix[1:5,1:5]
...
Alternatively, specifying `format = "igraph"` yields an *igraph* bipartite graph that includes legislator and bill characteristics as vertex attibutes:
```{r, echo = TRUE, results = 'hide'}
G <- incidence.from.congress(session = 115, types = "sjres", format = "igraph")
```
```{r}
G
```
