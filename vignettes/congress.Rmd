...
## Welcome {#welcome}
This vignette describes how to use the `incidentally` package to obtain data on bill sponsorship in US Congress, and how to use the `backbone` package to construct political networks from these data. For a more general introduction to the `incidentally` package, see [this companion vignette](incidentally.html).
...
```{r, echo = TRUE, results = 'hide', warning = FALSE}
B <- incidence.from.congress(session = 115, types = c("s", "sjres"), areas = "Health", format = "igraph")
```
```{r}
B
```
